...
import { Upload, Trash2, Image, Film, Eye, X, Lock, LogOut, ArrowLeft, Check } from 'lucide-react';
            ...
            >
                <div className="mb-8">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <h1 className="text-5xl font-black uppercase tracking-tighter leading-none">Admin<br />Access</h1>
                    </div>
                    <div className="w-full h-2 bg-[#ff4800]" />
                </div>

                <label className="block font-mono text-xs font-bold uppercase tracking-widest mb-2 text-black">Password</label>
                <input
                    type="password"
                    value={input}
                ...
                {error && (
                    <p className="mt-3 font-black text-sm uppercase text-[#ff4800]">Access Denied</p>
                )}

                <motion.button
                    whileHover={{ scale: 1.02, x: -3, y: -3, boxShadow: '6px 6px 0px 0px rgba(0,0,0,1)' }}
                    whileTap={{ scale: 0.98 }}
                    type="submit"
...
function MediaCard({ file, password, onDelete, onMetaChange }: { file: MediaFile; password: string; onDelete: () => void; onMetaChange: (name: string, field: 'displayName' | 'price' | 'category', value: string | number) => void }) {
    ...
    const saveMeta = useCallback(async (name: string, field: 'displayName' | 'price' | 'category', value: string | number) => {
        setSaving(true);
        try {
    ...
    const handleNameChange = (val: string) => {
        ...
        saveTimer.current = setTimeout(() => saveMeta(file.name, 'displayName', val), 600);
    };

    const handlePriceChange = (val: string) => {
        setPrice(val);
        clearTimeout(saveTimer.current);
        saveTimer.current = setTimeout(() => saveMeta(file.name, 'price', parseFloat(val) || 0), 600);
    };

    const handleCategoryChange = (val: string) => {
        setCategory(val);
        clearTimeout(saveTimer.current);
        saveTimer.current = setTimeout(() => saveMeta(file.name, 'category', val), 600);
    };
            ...
            >
                ...
                <div className="p-3 space-y-2 border-t-4 border-black">
                    ...
                    <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1 border-2 border-black px-2 py-1">
                            <span className="font-mono text-[10px] font-bold uppercase text-gray-500">₹</span>
                            <input
                                value={price}
                    ...
                    <div className="flex items-center gap-2 border-t-2 border-black pt-2 mt-1">
                        <span className="font-mono text-[9px] font-bold uppercase text-gray-500 flex-shrink-0">Season</span>
                        <select
                            value={category}
                        ...
                        {saving && (
                            <motion.div animate={{ opacity: [0, 1] }} className="text-[10px] font-mono text-gray-400 uppercase flex-shrink-0">...</motion.div>
                        )}
                    </div>
                </div>
                ...
                {file.type === 'video' && (
                    <div className="absolute top-2 right-2">
                        <span className="font-mono text-[10px] font-bold uppercase px-2 py-1 bg-[#ff4800] text-black border-2 border-black">▶</span>
                    </div>
                )}
            </motion.div>
                ...
                >
                    ...
                    >
                        ...
                        <div className="mt-4 flex items-center justify-center gap-6">
                            <p className="font-mono text-sm text-white uppercase tracking-wider">{preview.displayName}</p>
                            {preview.price > 0 && (
                                <span className="font-black text-lg text-[#ff4800]">₹{preview.price.toFixed(2)}</span>
                            )}
                            <span className="font-mono text-xs text-gray-400">· {formatSize(preview.size)}</span>
                        </div>
                    </motion.div>
                </div>
            )}
        ...
        return (
            <div className="border-8 border-black bg-white p-16 text-center">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="font-black text-3xl uppercase tracking-tighter text-black">No Media Yet</p>
                    <p className="font-mono text-sm uppercase tracking-widest text-gray-600">
        ...
        <div className="min-h-screen bg-[#E0E0E0] selection:bg-[#ff4800] selection:text-black">
            ...
            <nav className="sticky top-0 z-50 bg-black border-b-4 border-[#ff4800]">
                <div className="flex items-center justify-between px-6 py-4">
                    <a href="/" className="flex items-center gap-3 text-white font-black text-xl uppercase tracking-tighter hover:text-[#ff4800] transition-colors">
                        <ArrowLeft size={24} />
                        RJ GARMENT
                    </a>
            ...
            <footer className="border-t-8 border-black bg-black mt-32">
                <div className="max-w-7xl mx-auto px-6 py-8 flex flex-col md:flex-row justify-between items-center gap-4">
                    <span className="font-black text-xl uppercase tracking-tighter text-white">RJ GARMENT</span>
